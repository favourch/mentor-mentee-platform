RSpec.describe Api::V1::WishesController, type: :request do
  ...
  let!(:wishes) do
    create_list(
      ...
      meetinginterval_id: meetinginterval.id
    )
  end
    ...
    path '/api/v1/profile/wishes/' do
      ...
      post 'Create a wish for user you are logged in as' do
        ...
        parameter name: :wish, in: :body, schema: {
          ...
          required: %w[goal programminglanguage_id meetinginterval_id]
        }

        response '201', 'Wish created' do
          let(:wish) do
            {
              goal: 'Get better at programming',
              programminglanguage_id: programminglanguage.id,
              meetinginterval_id: meetinginterval.id
            }
          end
          run_test! do
            expect([json['goal'],
                    json['meetinginterval_id'],
                    json['programminglanguage_id']]).to eq(['Get better at programming',
                                                            meetinginterval.id,
        ...
        response '422', 'invalid request' do
          let(:wish) { {} }
          run_test! do
            expect(json['message']).to match(/Validation failed: \Programminglanguage must exist, \
Meetinginterval must exist, Goal can't be blank/)
          end
        end
      end
    end
    ...
    path '/api/v1/profile/wishes/{id}/' do
      ...
      delete 'Deletes  wish from user you are logged in as' do
        ...
        context 'Check that wish no longer exists after delete' do
          ...
          before do
            delete "/api/v1/profile/wishes/#{id}/",
                   headers: { Authorization: "Bearer: #{authorization}" },
                   params: updateparams, as: :json
            get "/api/v1/profile/wishes/#{id}/",
                headers: { Authorization: "Bearer: #{authorization}" },
                params: updateparams, as: :json
          end
        ...
        patch 'Update a wish belonging to user you are logged in as' do
          ...
          context 'Check that correct updates are made' do
            ...
            let(:updateparams) do
              { goal: 'Simply the best',
                programminglanguage_id: newlang.id,
                meetinginterval_id: newinterval.id }
            end
            before do
              patch "/api/v1/profile/wishes/#{id}/",
                    headers: { Authorization: "Bearer: #{authorization}" },
                    params: updateparams,
                    as: :json
              get "/api/v1/profile/wishes/#{id}/",
            ...
            it 'returns same params as updated' do
              expect([json['goal'],
                      json['meetinginterval_id'],
                      json['programminglanguage_id']]).to eq(['Simply the best', newinterval.id, newlang.id])
            end
          end
        end
      end
